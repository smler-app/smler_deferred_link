...
import 'package:smler_deferred_link/src/ios_clipboard_deep_link_result.dart';
...
/// Main entrypoint for the `smler_deferred_link` plugin.
///
/// Usage:
/// ```dart
/// try {
...
/// }
/// ```
class SmlerDeferredLink {
  static const MethodChannel _channel = MethodChannel(
    'io.smler.deferred_link',
  );
  ...
  static Future<ReferrerInfo> getInstallReferrerAndroid() async {
    ...
    if (!Platform.isAndroid) {
      throw UnsupportedError(
        'smler_deferred_link: Install Referrer is only available on Android.',
      );
    }
    ...
    try {
      ...
      if (raw is! Map) {
        ...
        throw StateError(
          'smler_deferred_link: Unexpected response type from platform: ${raw.runtimeType}',
        );
      }
      ...
      throw StateError(
        'smler_deferred_link: Unexpected error while reading install referrer: $e',
      );
    }
  }
  ...
  static Future<IosClipboardDeepLinkResult?> getInstallReferrerIos({
    ...
    if (!Platform.isIOS) {
      throw UnsupportedError(
        'smler_deferred_link: getInstallReferrerIos() is only intended for iOS.',
      );
    }
    ...
    try {
      ...
      throw PlatformException(
        code: e.code,
        message:
            'smler_deferred_link: Failed to read clipboard text on iOS: ${e.message}',
        details: e.details,
      );
    } catch (e) {
      throw StateError(
